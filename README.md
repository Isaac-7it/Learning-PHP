...
+ The Comparision operator (`==`) is used to compare two values for equality. However, when comparing floating-point numbers, it may not yield the expected result due to precision issues. Therefore, it is recommended to use a tolerance value (epsilon) when comparing floating-point numbers. Example
```php
$total = 0.1 + 0.1 + 0.1;
$expected = 0.3;
